...
const AdminVisaExtensionRequestManager = () => {
  ...
  const [students, setStudents] = useState([]); // Add student data state
  ...
  useEffect(() => {
    ...
    fetchStudents(); // Get student data
  }, []);
  ...
  const filteredExtensionRequests = useMemo(() => {
    return extensionRequests.filter((request) => {
      ...
      if (searchTerm) {
        ...
        if (
          !(
            request.extensionRequestId.toLowerCase().includes(term) ||
      ...
      if (filters.student) {
        ...
        const studentIdMatch = request.studentId
          .toLowerCase()
          .includes(studentTerm);
        const studentNameMatch = getStudentName(request.studentId)
          .toLowerCase()
          .includes(studentTerm);
  ...
  const getCurrentExtensionRequests = () => {
    ...
    return filteredExtensionRequests.slice(
      indexOfFirstRequest,
      indexOfLastRequest
    );
  };
  ...
  const nextPage = () => {
    if (
      currentPage < Math.ceil(filteredExtensionRequests.length / itemsPerPage)
    ) {
      setCurrentPage(currentPage + 1);
    }
  };
  ...
  const updateRequestStatus = async (requestId, status) => {
    try {
      const request = extensionRequests.find(
        (req) => req.extensionRequestId === requestId
      );
  ...
  const confirmDelete = async () => {
    try {
      ...
      await axiosInstance.delete(
        `/visa-extension-requests/${requestIdToDelete}`
      );

      // Update local state to remove the deleted request
      const updatedRequests = filteredExtensionRequests.filter(
        (req) => req.extensionRequestId !== requestIdToDelete
      );
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-600">
              Manage student visa extension requests • Total:{" "}
              {filteredExtensionRequests.length}
            </p>
          </div>
        </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentExtensionRequests.map((request) => (
                    <tr
                      key={request.extensionRequestId}
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        <FormButton
                          ...
                          variant="secondary"
                          onClick={() =>
                            deleteRequest(request.extensionRequestId)
                          }
                        >
                          Delete
                        </FormButton>
