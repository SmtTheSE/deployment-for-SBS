...
const ProfilePage = () => {
  ...
  const { profileImage, updateProfileImage, clearProfileImageCache } =
    useProfileImage();
  ...
  const handleFileChange = (event) => {
    ...
    axios
      .post(
        "http://localhost:8080/api/account/profile/upload-image",
        formData,
        {
          headers: {
  ...
  const handleCertificateUpload = () => {
    ...
    // Fix: Use axiosInstance instead of axios, and remove /api prefix
    axiosInstance
      .post("/academic/certificates/upload", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      })
      ...
      .catch((error) => {
        ...
        if (error.response) {
          ...
          if (error.response.status === 400) {
            alert(
              "Failed to upload certificate: Bad Request. The server may not have the upload endpoint implemented yet."
            );
            ...
            alert(
              `Failed to upload certificate: ${error.response.status} - ${error.response.statusText}`
            );
          }
        } else if (error.request) {
          // Request was made but no response received
          alert(
            "Failed to upload certificate: No response from server. Please check your connection."
          );
        } else {
          // Other errors
  ...
  const fetchGeneralInfo = (studentId) => {
    ...
    axiosInstance
      ...
      .then((res) => {
        const paymentStatus = res.data.some((p) => p.paymentStatus === 1)
          ? 1
          : 0;
    ...
    axiosInstance
      .get(`/academic/course-results/total-credits/${studentId}`)
      .then((res) =>
        setGeneralInfo((prev) => ({ ...prev, totalCredits: res.data }))
      )
    ...
    axiosInstance
      ...
      .then((res) => {
        const deadlines = res.data.map((item, idx) => ({
          ...
          deadline: new Date(item.assignmentDeadline).toLocaleDateString(
            "en-GB"
          ),
        }));
  ...
  return (
    <section>
      <Container>
        ...
        <div className="h-48 md:h-80 w-full overflow-hidden relative rounded-b-2xl">
              ...
              {profile && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                  <div className="pb-4 md:pb-0 border-b md:border-b-0 md:border-r border-gray-200">
                    <h3 className="text-sm uppercase text-gray-500 font-semibold tracking-wider">
                      Full Name
                    </h3>
              ...
              {deadlinesTotalPages > 1 && (
                <div className="flex justify-center mt-6">
                  <nav className="flex items-center gap-1">
                    <button
                      onClick={() =>
                        handleDeadlinesPageChange(deadlinesCurrentPage - 1)
                      }
          ...
          <div className="mb-8 bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-xl">
            ...
            <div className="p-6">
              <p className="text-gray-600 mb-5">
                Upload your previous certificates for credit exemption
                consideration.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center">
                ...
                {certificateFile && (
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm text-gray-700 truncate max-w-xs bg-gray-100 px-3 py-1 rounded-md">
      ...
      <CustomSuccessDialog
        ...
        message="Your certificate has been successfully uploaded for credit exemption consideration."
        confirmText="OK"
      />
    </section>
  );
