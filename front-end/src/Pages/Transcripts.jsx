...
const TranscriptFilter = ({ records, onFilterChange }) => {
  ...
  // 获取所有学期选项
  const semesters = [
    ...new Set(records.map((record) => record.semester)),
  ].sort();

  // 获取所有成绩选项
  const grades = [
    ...new Set(
      records.flatMap((record) => record.courses.map((course) => course.grade))
    ),
  ]
    .filter((grade) => grade !== "-")
    .sort();
...
const Transcripts = () => {
  ...
  const getTotalPages = (data) => {
    ...
    const totalRecords = data.reduce((total, record) => {
      return (
        total + (Array.isArray(record.courses) ? record.courses.length : 0)
      );
    }, 0);
  ...
  const handleFilter = (type, value) => {
    ...
    if (
      (!newFilters.semester && !newFilters.grade) ||
      (newFilters.semester === "" && newFilters.grade === "")
    ) {
      ...
      if (newFilters.semester) {
        filtered = filtered.filter(
          (record) => record.semester === newFilters.semester
        );
      }
        ...
        filtered = filtered
          .map((record) => {
            ...
            const filteredCourses = courses.filter((course) =>
              course.grade.includes(newFilters.grade)
            );
  ...
  return (
    <section>
      <Container className="p-4 md:p-10 flex flex-col lg:flex-row justify-between items-start gap-5">
        {/* Academic Records */}
        <div className="shadow-md p-5 rounded-md w-full lg:w-[70%] bg-white">
          ...
          <div className="block md:hidden space-y-4 mb-5">
            {paginatedRecords.length > 0 ? (
              paginatedRecords.map((record, index) => (
                <div
                  key={index}
                  className="bg-white p-4 rounded-lg shadow border border-gray-100 flex flex-col gap-2"
                >
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${getGradeColorClass(
                        record.grade
                      )}`}
                    >
                      {record.grade || "-"}
              ...
              <div className="text-center py-5 text-gray-500 bg-white rounded-lg shadow">
          ...
          <div className="hidden md:block overflow-x-auto">
            <table
              className={`w-full text-left mb-5 transition-opacity duration-150 ${
            ...
            >
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="py-3 xs:w-32">Grade</th>
                </tr>
              </thead>
              <tbody>
                {paginatedRecords.length > 0 ? (
                  paginatedRecords.map((record, index) => (
                    <tr
                      ...
                      key={index}
                    >
                      <td className="py-5 xs:text-[12px] md:text-sm">
                        {record.semester || ""}
                      </td>
                      <td className="py-5 xs:text-[12px] md:text-sm">
                        {record.course || "-"}
                      ...
                      <td className="py-5 xs:text-[12px] md:text-sm">
                        <span
                          className={`px-3 py-1 rounded-full xs:text-[10px] md:text-sm font-semibold ${getGradeColorClass(
                            record.grade
                          )}`}
                        >
                          {record.grade || "-"}
        ...
        {/* Request */}
        <div className="shadow-md rounded-md w-full lg:w-[30%] p-5 flex flex-col justify-between items-center bg-white">
                  ...
                  {requestType === 1 && (
                    <div className="mt-2 p-3 bg-blue-50 rounded-md text-sm text-blue-800 border border-blue-200">
                      ...
                      <ol className="list-decimal list-inside mt-1 space-y-1">
                        ...
                        <li>
                          Upon approval, the transcript will be issued with
                          official seal
                        </li>
                        <li>
                          You will be notified when it's ready for pickup or
                          mailing
                        </li>
                      </ol>
                    </div>
