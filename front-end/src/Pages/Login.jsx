...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Try to detect if we're in mock mode or having connection issues
      const isMockMode = import.meta.env.VITE_USE_MOCK === 'true' || 
                         (typeof window !== 'undefined' && window.location.hostname.includes('vercel.app'));
      
      if (isMockMode) {
        // Use mock login
        const loginResult = await mockLogin(email.split('@')[0], password);
        response = {
          data: {
            token: loginResult.token,
            role: loginResult.user.role.toLowerCase(),
            accountId: loginResult.user.id
          }
        };
      } else {
        // Try real login
