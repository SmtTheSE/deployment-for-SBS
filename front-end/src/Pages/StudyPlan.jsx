...
const FilterByDropDown = ({ filter, setFilter, subjPlans, onFilterChange }) => {
  // 获取所有唯一的年份和学期组合
  const uniqueYearSemOptions = [
    ...new Set(subjPlans.map((plan) => `Year ${plan.year} - Sem ${plan.sem}`)),
  ].sort((a, b) => {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      >
        <option value="none" className="text-font-light">
          All Years/Sems
        </option>
        {uniqueYearSemOptions.map((option) => (
          <option key={option} value={option} className="text-font-light">
            {option}
          </option>
        ))}
      </select>
...
const StudyPlan = () => {
  ...
  useEffect(() => {
    ...
    if (filter.filterBy !== "none") {
      ...
      if (match) {
        ...
        filtered = subjPlans.filter(
          (plan) => plan.year === year && plan.sem === sem
        );
      }
    }
  ...
  const getGroupedPlans = () => {
    ...
    const groups = [
      ...new Set(paginatedPlans.map((p) => `Year ${p.year} - Sem ${p.sem}`)),
    ].sort((a, b) => {
      const getVal = (str) => {
    ...
    <section className="p-4 md:p-10">
      <Container>
        <div className="flex flex-wrap justify-between items-center gap-5 mb-5">
          {academicInfos.map((info) => (
            ...
            >
              ...
              <div>
                <h5 className="font-light text-font-light text-sm uppercase tracking-wider">
                  {info.detail}
                </h5>
                <h1
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start gap-5">
          <div className="bg-white p-5 rounded-md w-full lg:w-2/3 shadow-md hover:shadow-lg transition-all duration-300">
            ...
            <table
              className={`w-full border-separate border-spacing-y-2 transition-opacity duration-150 ${
                    ...
                    {getGroupedPlans().map(({ group, groupItems }) => (
                      <React.Fragment key={group}>
                        <tr>
                          <td
                            colSpan="2"
                            className="bg-gray-200 font-bold p-2 rounded-t-lg"
                          >
                            {group}
                          </td>
                        </tr>
                          ...
                          >
                            ...
                            <td className="flex justify-between items-center border-l-5 border-border p-3 bg-blue-50 rounded-lg">
                              <h1>
                                ...
                                by {plan.lecturer || "Unknown Lecturer"}
                              </h1>
                              <span>
                                {plan.status === 1 ? (
                                  <FontAwesomeIcon
          ...
          <div className="bg-white p-5 w-full lg:w-1/3 rounded-md shadow-md hover:shadow-lg transition-all duration-300">
            <h1 className="text-2xl text-font-light uppercase pb-5 border-b border-border mb-5">
              Course History
            </h1>
            <div className="max-h-96 overflow-y-auto">
              {Array.isArray(filteredPlans) &&
              filteredPlans.filter((subj) => subj.status === 1).length > 0 ? (
                filteredPlans
                  .filter((subj) => subj.status === 1)
                  .map((subj) => (
                    <div
                      key={subj.id}
                      className="flex items-center bg-blue-50 border-l-5 border-border p-3 mb-3 rounded-lg hover:bg-blue-100 transition-colors duration-150"
                    >
                      <h1 className="flex-grow mr-2">
                        {subj.subject || "Unknown Subject"}
                      </h1>
